<?php
	...
	echo '<div class="zurf-single-author-tags clearfix" >';
	...
	if( !empty($author_desc) && zurf_get_option('general', 'blog-author', 'enable') == 'enable' ){
		...
		echo '<div class="zurf-single-author-caption zurf-info-font" >' . esc_html__('About the author', 'zurf') . '</div>';
	...
	if( zurf_get_option('general', 'blog-navigation', 'enable') == 'enable' ){
		...
		if( !empty($prev_post) ){
			$prev_post_html = get_previous_post_link(
				'<span class="zurf-single-nav-area-left" >%link</span>',
				'<span class="zurf-single-nav-area-background" ' . gdlr_core_esc_style(array('background-image' => get_post_thumbnail_id($prev_post->ID))) . ' ></span>' . 
				'<span class="zurf-single-nav-area-overlay" ></span>' . 
				'<span class="zurf-single-nav zurf-single-nav-left">' . 
				'<i class="arrow_left" ></i>' . 
				'<span class="zurf-text" >' . esc_html__( 'Previous Post', 'zurf' ) . '</span>' .
		...
		if( !empty($next_post) ){
			$next_post_html = get_next_post_link(
				'<span class="zurf-single-nav-area-right" >%link</span>',
				'<span class="zurf-single-nav-area-background" ' . gdlr_core_esc_style(array('background-image' => get_post_thumbnail_id($next_post->ID))) . ' ></span>' .
				'<span class="zurf-single-nav-area-overlay" ></span>' .
				'<span class="zurf-single-nav zurf-single-nav-right">' . 
				'<span class="zurf-text" >' . esc_html__( 'Next Post', 'zurf' ) . '</span>' . 
	...
	if( $enable_related_post == 'enable' && class_exists('gdlr_core_blog_style') ){
		...
		$related_post_args = array(
			...
			'blog-column-style' => zurf_get_option('general', 'related-post-blog-column-style', 'style-2'), // grid-with-frame
		...
		if( $query->have_posts() ){
			...
			echo '<div class="zurf-single-related-post-wrap" >';
			echo '<div class="zurf-single-related-post-container" >';
			echo '<div class="zurf-single-related-post-content zurf-item-rvpdlr" >';
			echo '<h3 class="zurf-single-related-post-title zurf-item-pdlr" >' . esc_html__('Related Posts', 'zurf') . '</h3>';
			...
			echo '</div>'; // zurf-single-related-post-content
			echo '</div>'; // zurf-single-related-post-container
	...
	if( comments_open() || get_comments_number() ){
		echo '<div class="zurf-comment-wrapper">';
		echo '<div class="zurf-comment-container zurf-container">';
		echo '<div class="zurf-comment-content zurf-item-pdlr">';
		comments_template();
		echo '</div>';
		echo '</div>';
		echo '</div>';

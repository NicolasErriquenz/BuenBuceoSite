<?php
	...
	if( zurf_get_option('general', 'blog-navigation', 'enable') == 'enable' ){
		$prev_post = get_previous_post_link(
			'<span class="zurf-single-nav zurf-single-nav-left">%link</span>',
			'<i class="arrow_left" ></i><span class="zurf-text" >' . esc_html__( 'Prev', 'zurf' ) . '</span>'
		);
		$next_post = get_next_post_link(
			'<span class="zurf-single-nav zurf-single-nav-right">%link</span>',
			'<span class="zurf-text" >' . esc_html__( 'Next', 'zurf' ) . '</span><i class="arrow_right" ></i>'
		);
		if( !empty($prev_post) || !empty($next_post) ){
			echo '<div class="zurf-single-nav-area clearfix" >' . $prev_post . $next_post . '</div>';
		}
	}

<?php
	...
	$header_style = zurf_get_option('general', 'header-plain-style', 'menu-right');
	$navigation_offset = zurf_get_option('general', 'fixed-navigation-anchor-offset', '');

	$header_wrap_class  = ' zurf-style-' . $header_style;
	$header_wrap_class .= ' zurf-sticky-navigation';
	if( $header_style == 'center-logo' || $body_layout == 'boxed' || 
		$body_margin != '0px' || $header_background_style == 'transparent' ){
		
		$header_wrap_class .= ' zurf-style-slide';
	}else{
		$header_wrap_class .= ' zurf-style-fixed';
	}
?>	
<header class="zurf-header-wrap zurf-header-style-plain <?php echo esc_attr($header_wrap_class); ?>" <?php
		if( !empty($navigation_offset) ){
			echo 'data-navigation-offset="' . esc_attr($navigation_offset) . '" ';
		}
	?> >
	<div class="zurf-header-background" ></div>
	<div class="zurf-header-container <?php echo esc_attr($header_container_class); ?>">
		...
		<div class="zurf-header-container-inner clearfix">
			<?php
				...
				}else if( $header_style == 'top-bar-logo' ){
			...
			<?php
				...
				if( has_nav_menu('main_menu') ){
					...
					if( ($enable_search || $enable_cart) && $header_style == 'menu-left' ){
						...
						if( $enable_cart ){
							...
							echo '<div class="zurf-main-menu-cart" id="zurf-menu-cart" >';
				...
				// menu right side
				$menu_right_class = '';
				if( in_array($header_style, array('center-menu', 'center-logo', 'splitted-menu', 'top-bar-logo')) ){
				...
				if( has_nav_menu('right_menu') || $enable_search || $enable_cart || $enable_right_button || !empty($custom_main_menu_right) || $side_content_menu ){
					...
					if( has_nav_menu('right_menu') && $header_style == 'splitted-menu'  ){
						echo '<div class="zurf-main-menu-left-wrap clearfix zurf-item-pdlr zurf-navigation-top" >';
						zurf_get_custom_menu(array(
							'container-class' => 'zurf-main-menu-right',
							'button-class' => 'zurf-right-menu-button zurf-top-menu-button',
							'icon-class' => 'fa fa-bars',

<?php
	...
	$header_style = zurf_get_option('general', 'header-bar2-navigation-align', 'center');
...
?>	
<header class="zurf-header-wrap zurf-header-style-bar zurf-style-2 <?php echo esc_attr($header_wrap_class); ?>" >
	...
	<div class="zurf-header-container clearfix <?php echo esc_attr($header_container_class); ?>">
		...
		<?php
			...
			$logo_right_text = '';
			$logo_right_text_content = zurf_get_option('general', 'logo-right-text');
			if( !empty($logo_right_text_content) ){
				$logo_right_text .= '<div class="zurf-logo-right-text-content" >';
				$logo_right_text .= gdlr_core_content_filter($logo_right_text_content);
				$logo_right_text .= '</div>';
			}

			if( !empty($logo_right_text) ){
				echo '<div class="zurf-logo-right-text zurf-item-pdlr clearfix" >' . gdlr_core_text_filter($logo_right_text) . '</div>';
			}
		?>
		</div>
	</div>
	<div class="zurf-navigation-bar-wrap zurf-navigation-header-style-bar zurf-style-2 <?php echo esc_attr($navigation_wrap_class); ?>" >
		
		<div class="zurf-navigation-container clearfix <?php echo esc_attr($header_container_class); ?>">
			<div class="zurf-navigation-background zurf-item-mglr" ></div>
			<?php
				$navigation_class = '';
			...
			<?php
				...
				if( has_nav_menu('right_menu') || $enable_search || $enable_cart || $enable_right_button || !empty($custom_main_menu_right) || $side_content_menu ){
					...
					if( $enable_right_button ){
						...
						echo '<a class="zurf-main-menu-right-button ' . esc_attr($button_class) . '" href="' . esc_url($button_link) . '" target="' . esc_attr($button_link_target) . '" >';

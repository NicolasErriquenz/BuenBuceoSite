 ...
 */
	...
	if( is_tax('portfolio_category') || is_tax('portfolio_tag') && class_exists('gdlr_core_pb_element_portfolio') ){
		...
		$sidebar_type = zurf_get_option('general', 'archive-portfolio-sidebar', 'none');
		$sidebar_left = zurf_get_option('general', 'archive-portfolio-sidebar-left');
		$sidebar_right = zurf_get_option('general', 'archive-portfolio-sidebar-right');		
	}else{
		$post_type = 'post';

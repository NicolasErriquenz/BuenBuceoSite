<?php
	...
	*/
	if( !function_exists('tourmaster_sanitize_contact_fields') ){
		function tourmaster_sanitize_contact_fields($fields){

			if( !empty($fields) ){
                foreach( $fields as $key => $value ){
					if( is_array($value) ){
						$fields[$key] = tourmaster_sanitize_contact_fields($value);
					}else{
	...
	if( !function_exists('tourmaster_save_email_locale') ){
		function tourmaster_save_email_locale($email){
			if( is_email($email) ){
				$locale = get_locale();
				$email_locales = get_option('tourmaster_email_locale', array()); 

				if( empty($email_locales[$email]) || $email_locales[$email] != $locale ){
					$email_locales[$email] = $locale;
					update_option('tourmaster_email_locale', $email_locales);
				}
			}
		}
	}
	add_filter('wpml_user_language', 'tourmaster_wpml_get_email_locale', 10, 2);
	if( !function_exists('tourmaster_wpml_get_email_locale') ){
		function tourmaster_wpml_get_email_locale($lang, $email){
			return substr(tourmaster_get_email_locale($lang, $email), 0, 2);
		}
	}
	if( !function_exists('tourmaster_get_email_locale') ){
		function tourmaster_get_email_locale($lang, $email) {
			$email_locales = get_option('tourmaster_email_locale', array()); 
			if( !empty($email_locales[$email]) ){
				return $email_locales[$email];
			}
    ...
    if( !function_exists('tourmaster_navigation_currency') ){
		function tourmaster_navigation_currency($ret = '', $settings = array()){ 
			...
			if( !empty($currencies) ){
				...
				$button .= '<ul>';
				if( !empty($tourmaster_currency) ){
					$button .= '<li><a href="' . esc_url(add_query_arg(array('currency' => strtolower($main_currency)))) . '" >' . strtoupper($main_currency) . '</a></li>';
				}
				foreach( $currencies as $currency ){
					if( $current_currency == strtolower($currency['title']) ) continue;
					$button .= '<li><a href="' . esc_url(add_query_arg(array('currency' => strtolower($currency['title'])))) . '" >' . strtoupper($currency['title']) . '</a></li>';
				}
				$button .= '</ul>';
	...
	if( !function_exists('tourmaster_wpml_posts') ){
		function tourmaster_wpml_posts($tour_id, $post_type = 'tour'){	
			global $sitepress;
			...
			// wpml is enable
				...
				$trid = $sitepress->get_element_trid($tour_id, 'post_' . $post_type);
	...
	// filter date 
	// time_offset is 60 * 60 * 24 - 1 = 86399
	if( !function_exists('tourmaster_filter_tour_date') ){
		function tourmaster_filter_tour_date( $dates, $month = '', $time_offset = 86399 ){

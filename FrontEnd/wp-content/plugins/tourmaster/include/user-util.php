	...
	if( !function_exists('tourmaster_woocommerce_country_code') ){
		function tourmaster_woocommerce_country_code($country){
			...
			switch($country){
				...
				case 'Saint Martin (Dutch part)': return 'SX';
				...
				case 'South Georgia/Sandwich Islands': return 'GS';
				...
				case 'United Arab Emirates (UAE)': return 'AE';
				case 'United Kingdom (UK)': return 'GB';
				case 'United States (US)': return 'US';
				case 'United States of America (USA)': return 'US';
				case 'United States (US) Minor Outlying Islands': return 'UM';
				...
				case 'Virgin Islands (US)': return 'VI';
		...
		function tourmaster_update_profile_avatar(){
			...
			if( !empty($uploaded_file) && empty($uploaded_file['error']) ){
				...
				if( !is_wp_error($cropped_image) ) {
					...
					if( !in_array(strtolower($ext), array('gif','jpg','jpeg','png')) ){ return; }
		...
		function tourmaster_get_user_breadcrumb(){
			...
			if( $main_page != 'dashboard' ){
				...
				if( empty($sub_page) ){
					...
					echo '<a class="tourmaster-user-breadcrumbs-item" href="' . esc_url($page_link) . '" >' . $main_nav_title . '</a>';
					echo '<span class="tourmaster-sep" >></span>';
				}	
			}
	...
	if( !function_exists('tourmaster_user_top_bar') ){
		function tourmaster_user_top_bar(){
			...
			if( is_user_logged_in() ){
				...
				$style = tourmaster_get_option('general', 'top-bar-login-style', 'style-1');
				$signup = tourmaster_get_option('general', 'top-bar-sign-up', 'enable');
				if( $style == 'style-1-2' ){ $style = 'style-1'; }
...
<div class="tourmaster-login-bottom" >
	...
	<a class="tourmaster-login-bottom-link" href="<?php echo esc_url(tourmaster_get_template_url('register')); ?>" ><?php echo esc_html__('Create an Account', 'tourmaster'); ?></a>
</div>
<?php
			if( !$echo ){
				$ret = ob_get_contents();
	...
	if( !function_exists('tourmaster_get_login_form2') ){
		function tourmaster_get_login_form2( $echo = true, $settings = array() ){
			$enable_membership = tourmaster_get_option('general', 'enable-membership', 'enable');
			if( $enable_membership == 'disable' ) return '';
...
<div class="tourmaster-login2-right" >
	...
	<div class="tourmaster-login2-right-content" >
		...
		?></div>
		<a class="tourmaster-button tourmaster-register-button" rel="nofollow" href="<?php 
			$register_url = tourmaster_get_template_url('register');
			if( !empty($settings['redirect']) ){
	...
	<?php if( !empty($settings['continue-as-guest']) ){ ?>
		...
		<a class="tourmaster-button tourmaster-continue-button" href="<?php 
			...
			echo esc_url(tourmaster_get_template_url($type)); 
		?>" ><?php esc_html_e('Continue As Guest', 'tourmaster'); ?></a>
	<?php } ?>
</div>
</div>
<?php
	...
	if( !function_exists('tourmaster_get_registration_form') ){
		function tourmaster_get_registration_form( $echo = true, $query_args = array() ){
			...
			do_action('tourmaster_registration_before_submit');
			...
			echo '<a class="tourmaster-register-bottom-link" href="' . esc_url(tourmaster_get_template_url('login')) . '" >' . esc_html__('Login', 'tourmaster') . '</a>';
			echo '</div>';
	...
	if( !function_exists('tourmaster_ajax_add_wish_list') ){
		function tourmaster_ajax_add_wish_list(){
			...
			if( is_user_logged_in() && !empty($_POST['tour-id']) ){
				...
				$_POST['tour-id'] = intval($_POST['tour-id']);
	...
	add_action('show_user_profile', 'tourmaster_extra_user_profile_fields');
	add_action('edit_user_profile', 'tourmaster_extra_user_profile_fields');
	if( !function_exists('tourmaster_extra_user_profile_fields') ){
		function tourmaster_extra_user_profile_fields( $user ) { 
?>	
			<h3><?php _e("Tourmaster", "blank"); ?></h3>
			<table class="form-table">
			<tr>
				<th><label for="birth_date"><?php _e("Birth Date"); ?></label></th>
				<td>
...
<?php 
	...
	add_action('personal_options_update', 'tourmaster_save_extra_user_profile_fields');
	add_action('edit_user_profile_update', 'tourmaster_save_extra_user_profile_fields');
	if( !function_exists('tourmaster_save_extra_user_profile_fields') ){
		function tourmaster_save_extra_user_profile_fields( $user_id ) {

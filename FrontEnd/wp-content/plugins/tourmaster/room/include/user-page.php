<?php
    ...
    if( !function_exists('tourmaster_room_user_nav_list') ){
		function tourmaster_room_user_nav_list($nav_list){
			...
			$enable_review = tourmaster_get_option('room_general', 'enable-review-submission', 'enable');
			if( $enable_review == 'enable' ){
				$nav_list['room-reviews'] = array(
			...
			if( !empty($results) ){	
				...
				foreach( $results as $result ){
					...
					if( in_array($result->order_status, array('pending', 'receipt-submitted', 'rejected')) ){
						$status .= '<a class="tourmaster-my-booking-action fa fa-remove" title="' . esc_html__('Cancel', 'tourmaster') . '" href="' . esc_url(add_query_arg(array('action'=>'remove', 'id'=>$result->id, 'order_type' => 'room-booking'))) . '" ';
	...
	if( !function_exists('tourmaster_get_room_invoice_price') ){
		function tourmaster_get_room_invoice_price( $booking_details, $price_breakdowns ){
            ...
            $ret .= '<span class="tourmaster-head">' . esc_html__('Grand Total Price', 'tourmaster') . '</span> ';

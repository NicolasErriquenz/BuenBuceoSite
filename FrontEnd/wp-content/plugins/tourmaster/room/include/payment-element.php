<?php
    ...
    if( !function_exists('tourmaster_room_price_sidebar') ){
        function tourmaster_room_price_sidebar( $price_breakdowns = array(), $step = 2, $payment_info = array(), $order_status = '', $currency = '' ){
            ...
            if( !empty($price_breakdowns['city-tax-rate']) && !empty($price_breakdowns['city-tax-amount']) ){
                if( strpos($price_breakdowns['city-tax-rate'], '%') !== false ){
                    echo '<div class="tourmaster-price clearfix" >';
                    echo '<span class="tourmaster-head" >' . sprintf(esc_html__('City Tax %s', 'tourmaster'), $price_breakdowns['city-tax-rate']) . '</span>';
                    echo '<span class="tourmaster-tail tourmaster-right" >';
                    echo tourmaster_money_format($price_breakdowns['city-tax-amount']);
                    echo '</span>';
                    echo '</div>';
                }else{
                    echo '<div class="tourmaster-price  clearfix" >';
                    echo '<span class="tourmaster-head" >' . sprintf(esc_html__('City Tax %s/Person/Night', 'tourmaster'), tourmaster_money_format($price_breakdowns['city-tax-rate'])) . '</span>';
                    echo '<span class="tourmaster-tail tourmaster-right" >';
                    echo tourmaster_money_format($price_breakdowns['city-tax-amount']);
                    echo '</span>';
                    echo '</div>';
                }
            }else if( !empty($price_breakdowns['city-tax-amount']) ){
                echo '<div class="tourmaster-price  clearfix" >';
                echo '<span class="tourmaster-head" >' . esc_html__('City Tax', 'tourmaster') . '</span>';
                echo '<span class="tourmaster-tail tourmaster-right" >';
                echo tourmaster_money_format($price_breakdowns['city-tax-amount']);
                echo '</span>';
                echo '</div>';
            ...
            if( $step == 2 ){
                ...
                $enable_full_payment = tourmaster_get_option('room_payment', 'enable-full-payment', 'enable');
                ...
                if( $enable_woocommerce_payment == 'disable' && $deposit_info['deposit_rate'] != 100 ){
                    ...
                    if( $enable_full_payment == 'disable' ){
                    ...
                    if( $enable_full_payment == 'enable' ){
                        ...
                        echo '<div class="tourmaster-room-pay-type" style="display: none;" >';
                        echo '<div class="tourmaster-room-pay-type-item tourmaster-deposit tourmaster-active" >';
                        echo '<i class="icon_check" ></i>';
                        echo sprintf(esc_html__('Pay %d%% Deposit', 'tourmaster'), $deposit_info['deposit_rate']);
                        echo '</div>';
                        echo '</div>';
                ...
                echo tourmaster_room_get_paypal_jssdk_payment_variable($price_breakdowns, $deposit_info, $currency);
    ...
    if( !function_exists('tourmaster_room_price_sidebar_payment') ){
        function tourmaster_room_price_sidebar_payment($order_status = ''){
            ...
            $paypal_jssdk = false;
			if( in_array('paypal', $payment_method) ){
				$paypal_method = tourmaster_get_option('room_payment', 'paypal-method', 'standard');
				if( $paypal_method == 'js-sdk' ){
					$paypal_jssdk = true;
					$payment_method = array_diff($payment_method, array('paypal'));
				}
			}
            ...
            if( (empty($order_status) && $need_admin_approval) || 
                ...
                $custom_payment_enable = apply_filters('tourmaster_room_custom_payment_enable', false, $payment_method);
                if( $enable_woocommerce_payment == 'enable' ){
                ...
                }else if( $custom_payment_enable || in_array('paypal', $payment_method) || in_array('credit-card', $payment_method)  ){
                    ...
                    $payments_title = apply_filters('tourmaster_room_payment_methods', $payments_title);
                    
                    if( in_array('paypal', $payment_method) ){
                        $p_service_fee = tourmaster_get_option('room_payment', 'paypal-service-fee', '');
                    }
                    if( in_array('credit-card', $payment_method) ){
                        $c_service_fee = tourmaster_get_option('room_payment', 'credit-card-service-fee', '');
                    }
                    if( !empty($p_service_fee) && !empty($c_service_fee) ){
                        echo '<div style="font-size: 13px; margin-bottom: 15px; " >';
                            echo sprintf(esc_html__('Additional %s%% is charged for PayPal payment and %s%% is charged for Credit Card payment', 'tourmaster'), $p_service_fee, $c_service_fee);
                            echo '</div>';
                    }else if( !empty($p_service_fee) ){
                        echo '<div style="font-size: 13px; margin-bottom: 15px; " >';
                        echo sprintf(esc_html__('Additional %s%% is charged for PayPal payment.', 'tourmaster'), $p_service_fee);
                        echo '</div>';
                    }else if( !empty($c_service_fee) ){
                        echo '<div style="font-size: 13px; margin-bottom: 15px; " >';
                        echo sprintf(esc_html__('Additional %s%% is charged for Credit Card payment.', 'tourmaster'), $service_fee);
                        echo '</div>';
                    }
    ...
    if( !function_exists('tourmaster_room_single_price_breakdown') ){
        function tourmaster_room_single_price_breakdown( $booking_detail = array(), $price_breakdown = array(), $room_number = 0 ){
            ...
            echo '<h4 class="tourmaster-title">' . esc_html__('Price Breakdown', 'tourmaster') . '</h4>';
    ...
    if( !function_exists('tourmaster_room_price_summary') ){
        function tourmaster_room_price_summary( $booking_details = array(), $price_breakdowns = array() ){
            ...
            for( $i = 0; $i < sizeof($booking_details); $i++ ){
                ...
                $translated_id = apply_filters('wpml_object_id', $booking_detail['room_id'], 'room', true);

                echo '<div class="tourmaster-room-price-summary-block" >';
                echo '<div class="tourmaster-room-price-summary-room-title" >' . get_the_title($translated_id) . '</div>';
                ...
                for( $j = 0; $j < $booking_detail['room_amount']; $j++ ){
                    ...
                    if( !empty($room_option['room-service']) ){
                        ...
                        if( !empty($price_breakdown['city-tax-rate']) && !empty($price_breakdown['city-tax-amount']) ){
                            echo '<div class="tourmaster-city-tax-total clearfix" >';
                            echo '<div class="tourmaster-head" >' . sprintf(esc_html__('City Tax %s/Person/Night', 'tourmaster'), tourmaster_money_format($price_breakdown['city-tax-rate'])) . '</div>';
                            echo '<div class="tourmaster-tail" >' . tourmaster_money_format($price_breakdown['city-tax-amount']) . '</div>';
                            echo '</div>';
                        }
    ...
    if( !function_exists('tourmaster_get_room_booking_price_breakdown') ){
		function tourmaster_get_room_booking_price_breakdown($booking_details, $price_breakdowns){
            ...
            if( !empty($price_breakdowns['city-tax-rate']) && !empty($price_breakdowns['city-tax-amount']) ){
                if( strpos($price_breakdowns['city-tax-rate'], '%') !== false ){
                    $ret .= '<div class="tourmaster-price-breakdown-city-tax" >';
                    $ret .= '<span class="tourmaster-head" >' . sprintf(esc_html__('City Tax %s', 'tourmaster'), $price_breakdowns['city-tax-rate']) . '</span>';
                    $ret .= '<span class="tourmaster-tail tourmaster-right" >';
                    $ret .= tourmaster_money_format($price_breakdowns['city-tax-amount']);
                    $ret .= '</span>';
                    $ret .= '</div>';
                }else{
                    $ret .= '<div class="tourmaster-price-breakdown-city-tax" >';
                    $ret .= '<span class="tourmaster-head" >' . sprintf(esc_html__('City Tax %s/Person/Night', 'tourmaster'), tourmaster_money_format($price_breakdowns['city-tax-rate'])) . '</span>';
                    $ret .= '<span class="tourmaster-tail tourmaster-right" >';
                    $ret .= tourmaster_money_format($price_breakdowns['city-tax-amount']);
                    $ret .= '</span>';
                    $ret .= '</div>';
                ...
                $ret .= '<div class="tourmaster-price-breakdown-city-tax" >';
                $ret .= '<span class="tourmaster-head" >' . esc_html__('City Tax', 'tourmaster') . '</span>';
                $ret .= '<span class="tourmaster-tail tourmaster-right" >';
                $ret .= tourmaster_money_format($price_breakdowns['city-tax-amount']);
                $ret .= '</span>';
                $ret .= '</div>';
    ...
    if( !function_exists('tourmaster_room_ajax_pay_now') ){
        function tourmaster_room_ajax_pay_now(){
            ...
            $data = tourmaster_process_post_data($_POST['data']);
            $data['contact_info'] = tourmaster_sanitize_contact_fields($data['contact_info']);
            ...
            if( !empty($data['contact_info']['email']) ){
                tourmaster_save_email_locale($data['contact_info']['email']);
            }
            ...
            for( $i = 0; $i < sizeof($booking_details); $i++ ){          
                ...
                for( $j = 0; $j < intval($booking_details[$i]['room_amount']); $j++ ){
                    ...
                    tourmaster_insert_room_buffer($data);
                }
            }
            ...
            if( $payment_method == 'paypal' ){
                ...
                if( $paypal_method == 'js-sdk' ){
                    $payment_content = '';
                    $payment_order_id = empty($_POST['order_data']['id'])? '': $_POST['order_data']['id'];
					tourmaster_room_set_paypal_jssdk_payment_status($order_id, $payment_order_id);
                }else{
        ...
        function tourmaster_room_ajax_pay_woocommerce(){
            ...
            if( empty($contact_detail['required-billing']) || $contact_detail['required-billing'] == 'false' ){
                $billing_address = array(
                    ...
                    'city'       => '',
                    'state'      => '',
                    'postcode'   => empty($contact_detail['zipcode'])? '': $contact_detail['zipcode'],
                    'phone'  => empty($contact_detail['phone'])? '': $contact_detail['phone'],
                ...
                $billing_address = array(
                    ...
                    'state'      => '',
                    'postcode'   => empty($contact_detail['zipcode'])? '': $contact_detail['zipcode'],
                    'phone'  => empty($contact_detail['billing_phone'])? '': $contact_detail['billing_phone'],
                    'country'    => empty($contact_detail['billing_country'])? '': tourmaster_woocommerce_country_code($contact_detail['billing_country']),
                );
            }

            $wc_order = wc_create_order();
            $wc_order->update_meta_data('tourmaster-room-booking-info', tourmaster_get_woocommerce_checkout_order_meta($booking_details));
    ...
    add_action('woocommerce_order_item_meta_end', 'tourmaster_woocommerce_order_item_meta_end', 10, 3);
    if( !function_exists('tourmaster_woocommerce_order_item_meta_end') ){
        function tourmaster_woocommerce_order_item_meta_end($item_id, $item, $order){
            $order_meta = $order->get_meta('tourmaster-room-booking-info');
            if( !empty($order_meta) ){
                echo '<div class="tourmaster-wc-room-order-detail" >' . $order_meta . '</div>';
            }
        }
    }
    if( !function_exists('tourmaster_get_woocommerce_checkout_order_meta') ){
        function tourmaster_get_woocommerce_checkout_order_meta($booking_details = array()){
            $ret = '';
			for( $i = 0; $i < sizeof($booking_details); $i++ ){
				$booking_detail = $booking_details[$i];
				$ret .= '<div class="tourmaster-my-booking-single-field clearfix" >';
				$ret .= '<div class="tourmaster-head tourmaster-full">' . get_the_title($booking_detail['room_id']) . '</div> ';
				...
				$location = get_the_terms($booking_details[$i]['room_id'], 'room_location'); 
				if( !empty($location) ){
					$ret .= '<div class="tourmaster-tail tourmaster-indent">' . sprintf(esc_html__('Location : %s', 'tourmaster'), $location[0]->name) . '</div>';
				}
				$ret .= '</div>';
			}
    ...
    if( !function_exists('tourmaster_insert_room_buffer') ){
        function tourmaster_insert_room_buffer($data){

            global $wpdb;
            $sql  = "SELECT * FROM {$wpdb->prefix}tourmaster_room_settings ";
            $sql .= $wpdb->prepare("WHERE room_id = %d ", $data['room_id']);
            $sql .= "AND buffer_number > 0 ";
            $sql .= $wpdb->prepare("AND date_list LIKE %s ", '%' . $data['start_date'] . '%');
            $result = $wpdb->get_row($sql);

            // check if end date in buffer day
            if( !empty($result) && !empty($result->buffer_day) ){
                $buffer_day = intval($result->buffer_number);
                $cleaning_days = explode(',', $result->buffer_day);
                $end_date_day = date('N', strtotime($data['end_date']));
                
                if( in_array($end_date_day, $cleaning_days) ){
                    // do nothing in cleaning day
                }else{
                    $data['start_date'] = $data['end_date'];
                    $data['end_date'] = date('Y-m-d', strtotime($data['end_date']) + (86400 * intval($buffer_day)));
                    for( $i = 1; $i <= $buffer_day; $i++ ){
                        $end_date_day = date('N', strtotime($data['start_date']) + (86400 * $i));
                        if( in_array($end_date_day, $cleaning_days) ){
                            $data['end_date'] = date('Y-m-d', strtotime($data['start_date']) + (86400 * $i));
                        }
                    }
                    $data['buffer'] = 1;

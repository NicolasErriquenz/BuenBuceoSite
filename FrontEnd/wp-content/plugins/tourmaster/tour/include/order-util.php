	...
	if( !function_exists('tourmaster_order_edit_form') ){
		function tourmaster_order_edit_form($tid, $type = '', $result = '', $tour_option = array() ){
			...
			if( $type == 'new_order' ){
				...
				if( !empty($date_price['pricing-method']) && $date_price['pricing-method'] == 'group' ){
				...
				if( !empty($extra_booking_info) ){
					...
					foreach($extra_booking_info as $extra_info){
						$value = empty($booking_detail[$extra_info['slug']])? '': $booking_detail[$extra_info['slug']];
						
						$ret .= tourmaster_get_form_field(array(
							'title' => $extra_info['title'] . ' :',
							'echo' => false,
							'slug' => $extra_info['slug'],
							'type' => 'text'
						), 'order-edit', $value);
					}
				}
					...
					foreach( $tour_option['tour-service'] as $service_id ){
						...
						$ret .= tourmaster_get_form_field(array(
							...
							'pre-input' => '<div class="tourmaster-price-edit-amount" >' . 
								...
								'<span> x </span>' . 
							'</div>',
							'description' => sprintf(esc_html__('Price per %s (Fill only number).', 'tourmaster'), $pers[$service_option['per']])
		...
		function tourmaster_order_edit(){
			...
			// additional notes
				...
				if( !empty($data) ){
					$result = tourmaster_get_booking_data(array('id' => $data['tid']), array('single' => true));
					$contact_detail = empty($result->contact_info)? array(): json_decode($result->contact_info, true);
					$contact_detail['additional_notes'] = empty($data['additional_notes'])? '': $data['additional_notes'];

					$booking_detail = empty($result->booking_detail)? array(): json_decode($result->booking_detail, true);
					$booking_detail['additional_notes'] = $contact_detail['additional_notes'];
					...
					if( !empty($extra_booking_info) ){
						...
						foreach($extra_booking_info as $extra_info){
							$booking_detail[$extra_info['slug']] = empty($data[$extra_info['slug']])? '': $data[$extra_info['slug']];
						}
					}

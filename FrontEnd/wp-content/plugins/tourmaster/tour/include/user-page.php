			...
			if( !empty($results) ){	
				...
				foreach( $results as $result ){
					...
					if( in_array($result->order_status, array('pending', 'receipt-submitted', 'rejected')) ){
						$status .= '<a class="tourmaster-my-booking-action fa fa-remove" title="' . esc_html__('Cancel', 'tourmaster') . '" href="' . esc_url(add_query_arg(array('action'=>'remove', 'id'=>$result->id))) . '" ';

	...
	if( !class_exists('tourmaster_tour_style') ){
		class tourmaster_tour_style{
			...
			function tour_title( $args, $title_front = '', $title_back = '' ){
				...
				$ret .= '<a href="' . esc_url(get_permalink()) . '" >' . $title_front . get_the_title() . $title_back . '</a>';
				$ret .= '</h3>';
			...
			function get_thumbnail( $args, $has_content = true ){
				...
				if( !empty($feature_image) ){
					...
					if( $thumbnail_link_type == 'lightbox-to-video' ){
						...
						$ret .= tourmaster_get_image($feature_image, $args['thumbnail-size'], array(
							'image-class' => empty($args['image-class'])? '': $args['image-class']
						));
						$ret .= '</a>';
					}else{
						$ret .= '<a href="' . esc_url(get_permalink()) . '" >';
						$ret .= tourmaster_get_image($feature_image, $args['thumbnail-size'], array(
							'image-class' => empty($args['image-class'])? '': $args['image-class']
						));
						$ret .= '</a>';
					...
					if( !empty($args['thumbnail-category']) ){
						$ret .= $this->get_info(array('category'), array('no-wrapper' => true));
					}
					if( !empty($args['thumbnail-tag']) ){
						$ret .= $this->get_info(array('tag'), array('no-wrapper' => true));
					}
					$ret .= '</div>';
			...
			// tour rating
			function get_rating( $style = 'widget', $hide_on_empty = false, $sync_height_offset = false ){
				...
				if( !empty($rating['reviewer']) ){
					...
					$ret .= $sync_height_offset? ' data-sync-height-offset': '';
					$ret .= ' >';
			...
			function get_price( $settings = array() ){
				$ret = apply_filters('tourmaster_tour_style_get_price', '', $settings);
				if( !empty($ret) ) return $ret;
			...
			function get_info( $options = array(), $args = array() ){
				...
				foreach( $options as $type ){
					switch( $type ){
						case 'category': 
							$term_list = get_the_term_list(get_the_ID(), 'tour_category', '', '<span class="tourmaster-sep">,</span> ' , '' );
							if( !empty($term_list) ){
								$ret .= '<div class="tourmaster-tour-info tourmaster-tour-category" >' . $term_list . '</div>';
							}
			...
			function tour_full( $args ){
				...
				$ret .= '<a class="tourmaster-tour-view-more" href="' . esc_url(get_permalink()) . '" >' . esc_html__('View Details', 'tourmaster') . '</a>';
			...
			function tour_grid( $args ){
				...
				if( in_array($grid_style, array('style-6')) ){
					$args['thumbnail-ribbon'] = true;
				}else if( in_array($grid_style, array('style-7')) ){
					$args['thumbnail-ribbon'] = true;
					$args['thumbnail-tag'] = true;
				}
				$ret .= $this->get_thumbnail($args);
				...
				if( $grid_style == 'style-4' ){
					...
					if( !empty($args['tour-rating']) && $args['tour-rating'] == 'enable' ){
						if( $grid_style == 'style-3' ){
							$ret .= $this->get_rating('widget', true, true);
						}else{
							$ret .= $this->get_rating('widget', true);
						}
					} 
				...
				if( $grid_style == 'style-5' ){
					$ret .= '<div class="tourmaster-tour-read-more-wrap" data-sync-height-offset >';
					$ret .= '<a class="tourmaster-tour-read-more" href="' . esc_url(get_permalink()) . '" >' . esc_html__('Read More', 'tourmaster') . '</a>';
					$ret .= '</div>';
				}
				$ret .= '</div>'; // tourmaster-tour-content-wrap
				...
				if( empty($args['display-price']) || $args['display-price'] == 'enable' ){
					if( $args['price-position'] == 'bottom-bar' ){
						...
						if( !empty($post_meta['tour-price-text']) ){
							$decimal_digit = apply_filters('tourmaster_tour_item_price_decimal_digit', 0);
							$decimal_digit = intval($decimal_digit);
							...
							$ret .= '<span class="tourmaster-tour-price">' . tourmaster_money_format($post_meta['tour-price-text'], $decimal_digit) . '</span>';
							if( !empty($post_meta['tour-price-discount-text']) ){
								$ret .= '<span class="tourmaster-tour-discount-price" >';
								$ret .= tourmaster_money_format($post_meta['tour-price-discount-text'], $decimal_digit);
								$ret .= '</span>';
